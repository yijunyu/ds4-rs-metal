// Shim override of kernel_dsv4_hc_split_weighted_sum_norm4 (dsv4_hc.metal:371)
// with [[max_total_threads_per_threadgroup(1024)]].
//
// The raw upstream pipeline compiles register-heavy (the tid==0 sinkhorn block
// keeps r0..r3 + col_inv live) and its maxTotalThreadsPerThreadgroup drops
// below 1024 (832 on M1 Ultra release; 512 under MTL_SHADER_VALIDATION). The
// hosts (deferred.rs hc_split_weighted_sum_norm4 / _into_k) clamp the dispatch
// to the pipeline limit, but every reduced threadgroup shape changes the fp32
// reduction order vs the trusted 1024-lane decode baseline. The attribute
// forces the compiler to spill so 1024 lanes always fit — keeping the chunk
// prefill bit-identical to the per-token path on any device, in any
// validation mode. Body is verbatim upstream; loops are stride-based so the
// host clamps remain correct if the limit ever drops anyway.
[[max_total_threads_per_threadgroup(1024)]]
kernel void ds4_dsv4_hc_split_weighted_sum_norm4(
